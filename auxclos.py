...
def from_CASA_syntax(list0,totlist,whatami,default):
 ...
 if whatami=='spw':
   ...
   try:
    indices = [totlist.index(elem) for elem in list0] # check if all elements exist
...
def ClosComp(CDATA,MDATA,SelBas,baselines,q):
 ...
 if len(q)==3: #triplet. Hence, phase closure.
   ...
   phase = False
   basels = [[q[0],q[1]],[q[2],q[3]],[q[0],q[2]],[q[1],q[3]],[q[0],q[3]],[q[1],q[2]]]
 ...
 if len(filter(lambda x: x<0, irows))>0: # One or more baselines are missing.
     return [[False],[]]   # Returns a null result.

 else: # All baselines have data.

  Xvis = [CDATA[:,:,irow] for irow in irows]
  if type(MDATA) is float:  # There aren't model data. Use a point source.
  ...
  if phase:  # closure phases (1 closure per triplet)
   ...
   BisPecX *= np.conjugate(XvisMod[0]*XvisMod[1]*np.conjugate(XvisMod[2]))
...
# (define closures such that the residuals are >=0.0, on average):
   ...
   b1=[b[0],b[1],b[3],b[4]] ; b2 = [b[2],b[3],b[4],b[5]] ; b3 = [b[0],b[1],b[2],b[3]]
